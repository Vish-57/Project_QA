build_chunks <- function(parsed, chunk_chars = 6000L, overlap = 400L) {
  text <- parsed$text
  if (!nzchar(text)) {
    return(data.frame(id = integer(), heading = character(), start_char = integer(), end_char = integer(), text = character(), stringsAsFactors = FALSE))
  }
  ...
  if (use_sections) {
    ...
    spans <- data.frame(heading = sections$heading, start = starts, end = ends, stringsAsFactors = FALSE)
    chunks <- list()
    for (i in seq_len(nrow(spans))) {
      span_text <- substr(text, spans$start[i], spans$end[i])
      if (nchar(span_text) <= chunk_chars) {
        chunks[[length(chunks) + 1L]] <- list(heading = spans$heading[i], start = spans$start[i], end = spans$end[i], text = span_text)
      } else {
    ...
    chunks <- lapply(seq_along(sub), function(j) list(
      heading = paste0("Chunk ", j), start = attr(sub, "starts")[j], end = attr(sub, "ends")[j], text = sub[j]
    ))
  }
  data.frame(
    id = seq_along(chunks),
...
size_chunks <- function(text, chunk_chars, overlap) {
  n <- nchar(text)
  if (n <= chunk_chars) { out <- text; attr(out, "starts") <- 1L; attr(out, "ends") <- n; return(out) }
  step <- chunk_chars - overlap
  starts <- seq(1L, n, by = step)
  ends <- pmin(starts + chunk_chars - 1L, n)
  out <- mapply(function(s, e) substr(text, s, e), starts, ends, USE.NAMES = FALSE)
