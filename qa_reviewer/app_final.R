...
server <- function(input, output, session) {
  ...
  output$app_shell <- renderUI({
    ...
    tagList(
      tabsetPanel(id = "main_tab",
        tabPanel(icon("file-text"), " Review",
          sidebarLayout(
        ...
        tabPanel(icon("clock-rotate-left"), " History",
          div(class = "card",
        ...
        tabPanel(icon("lightbulb"), " Insights",
          div(class = "card",
    ...
    tagList(
      ...
      div(class = "card",
        div(style = "display:flex; gap:8px;",
          ...
          downloadButton("dl_pdf", icon("file-pdf"), " Report (.pdf)", class = "btn-outline-danger btn-sm"),
  ...
  output <- downloadHandler(
    filename = function() paste0("qa_report_", format(Sys.time(), "%Y%m%d_%H%M%S"), ".pdf"),
    content = function(file) {
      showNotification("PDF export is not yet implemented.", type = "warning")
    }
  )
  # --- History ---
